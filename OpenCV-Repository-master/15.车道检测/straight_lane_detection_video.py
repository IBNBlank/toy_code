# -*- coding: utf-8 -*-
# @Author: IBNBlank
# @Date:   2019-05-01 10:55:03
# @Last Modified by:   IBNBlank
# @Last Modified time: 2019-05-01 14:16:06
...
def get_lanes(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    left_results = least_squares(left_points, 320, img.shape[0])
    ...
    right_results = least_squares(right_points, 320, img.shape[0])
...
if __name__ == '__main__':
    # get video #
    file_path = "video\\cv2_white_lane.mp4"
    ...
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret is True:
            img_detection = process(frame)
            out.write(img_detection)
        else:
            break
